...
//EPWM Module 1
    ...
    // Event trigger
    // Proženje ADC-ja, triggering ADC
    EPwm1Regs.ETSEL.bit.SOCASEL = 2;    //sproži prekinitev na periodo
    EPwm1Regs.ETPS.bit.SOCAPRD = 1;     //ob vsakem prvem dogodku, each first event
    EPwm1Regs.ETCLR.bit.SOCA = 1;       //clear possible flag
    EPwm1Regs.ETSEL.bit.SOCAEN = 1;     //enable ADC Start Of conversion
    // Proženje prekinitve, triggering break
    EPwm1Regs.ETSEL.bit.INTSEL = 2;             //sproži prekinitev na periodo, start interruption in the period
    EPwm1Regs.ETPS.bit.INTPRD = PWM_INT_PSCL;   //ob vsakem prvem dogodku, each first event
...
* The feature that sets the period, in order to achieve the desired period,
* and the exact per cycle exactly
* return: void
* arg1: zelena perioda
**************************************************************/
